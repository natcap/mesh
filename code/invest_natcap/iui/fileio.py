...
class LastRunHandler(JSONHandler):
    def __init__(self, modelname, version=None):
        ...
        # Custom code
        try:
            set_folder = os.path.join(config.global_folder, 'output/model_setup_runs', modelname)

            uri = '%s_setup_file.json' % modelname
            rendered_path = os.path.join(set_folder, uri)
            JSONHandler.__init__(self, rendered_path)
        except:
            print 'Unable to find config.global_folder'
