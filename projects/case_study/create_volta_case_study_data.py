status = 'replacement for process_inputs_for_volta_ssi_example.py. '
...
aoi_uri = os.path.join(input_folder, 'aoi.shp')
# LULC Taken from MESH

precip_source_uri = os.path.join(base_data_dir, 'worldclim/baseline/30s/bio_12.tif')
precip_uri = os.path.join(output_folder, 'precip.tif')
nd.clip_raster_by_shp(precip_source_uri, precip_uri, aoi_uri)
...
def old_code():
    ...
    precip_source_uri = project_dir + 'Base_Data/climatic/baseline_bio12_Annual_Precipitation.tif'
    ...
    coastline_40km_buffer_uri = project_dir + 'Base_Data/physical/coastline_40km_buffer.shp'
